...
public class StreamForker<T> {
    ...
    private ForkingStreamConsumer<T> build() {
        ...
        Map<Object, Future<?>> actions =
                forks.entrySet().stream().reduce(
                        ...
                        (map, e) -> {
                            map.put(e.getKey(),
                                    StreamForker.this.getOperationResult(queues, e.getValue()));
                            return map;
                        },
    ...
    private Future<?> getOperationResult(List<BlockingQueue<T>> queues, Function<Stream<T>, ?> f) {
        ...
        Stream<T> source = StreamSupport.stream(spliterator, false);//非并行流
        return CompletableFuture.supplyAsync( () -> f.apply(source) );//异步执行
    }
